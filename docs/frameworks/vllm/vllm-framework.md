...
**Related pages:** [vLLM Continuous Batching](vllm-continuous-batching/index.md), [SGLang: Structured Language Model Programs](../sglang/index.md)
...
**How:** [PagedAttention](../../terms/pagedattention.md) stores [KV cache](../../terms/kv-cache.md) in non-contiguous fixed-size blocks, with logical-to-physical [block-table](../../terms/block-table.md) mapping, on-demand allocation, and copy-on-write sharing — analogous to OS paging.
...
This is related to SGLang's later RadixAttention design, but vLLM's paper focuses on block-level memory management and predefined sharing patterns, while [SGLang](../sglang/index.md) focuses on automatic radix-tree KV reuse across structured language-model programs.
...
Compared with [SGLang](../sglang/index.md), vLLM is lower in the stack. vLLM is an inference-serving engine centered on paged KV-cache memory management. SGLang is a programming and runtime framework for structured multi-call language-model programs, and its RadixAttention generalizes KV-cache reuse around prefix trees and cache-aware scheduling.
