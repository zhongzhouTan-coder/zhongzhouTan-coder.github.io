...
KV-cache quantization is a separate concern from the ModelSlim weight schemes above: it quantizes the K/V tensors that the FIA op reads back on every step. On the GQA/FIA path this is **C8 — static per-channel INT8 KV cache** (the QuaRot-style scheme), not FP8.
