...
**What:** In vllm-ascend, the DeepSeek-V4 [Lightning Indexer's](../../terms/lightning-indexer.md) key cache and query are always stored in 8-bit — **INT8 + FP16 per-token-head scales on non-A5 devices, [FP8](../../terms/fp8.md) e4m3fn + FP32 scales on A5** — so the indexer's sparse top-k selection never reads a full-precision [KV cache](../../terms/kv-cache.md).
