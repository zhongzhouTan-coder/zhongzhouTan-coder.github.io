...
vLLM's plugin system discovers vllm-ascend entirely through Python entry points declared in `setup.py`:
...
- `patch_fused_moe.py`: Replaces the upstream `FusedMoE` factory with `AscendMoERunner` before model import
- `patch_kv_connector.py`: Adapts KV transfer for PD (prefill-decode) disaggregation
- `patch_distributed.py`: Adapts `all_reduce` for 310P device type
...
Ascend-specific parallel groups (`parallel_state.py`) extend vLLM's standard TP/PP/DP groups with fine-grained groups: `_MC2` for MoE EP-like communication, `_MLP_TP`/`_OTP`/`_LMTP`/`_EMBED_TP` for component-specific tensor parallelism, `_P_TP` for PD disaggregation, and `_DYNAMIC_EPLB` for expert-parallel load balancing.
...
- **`AscendCompiler`**: Custom compile backend (`compilation/compiler_interface.py`) that uses `aot_autograd` + Ascend's `PassManager` for graph fusion instead of TorchInductor
...
|---|---|
| `ops/mla.py` | Multi-head Latent Attention for DeepSeek V2/V3 |
| `ops/dsa.py` | DeepSeek Sparse Attention kernels |
| `ops/linear.py` | Fractal-format linear layers |
| `ops/rotary_embedding.py` | RoPE with DeepSeek scaling |
| `ops/rope_dsv4.py` | Complex exponential RoPE for V4 |
| `ops/layernorm.py` | Custom RMSNorm |
| `ops/activation.py` | AscendQuickGELU, AscendSiluAndMul |
| `ops/fused_moe/` | Fused MoE with Ascend optimizations |
| `ops/vocab_parallel_embedding.py` | Ascend-optimized embedding |
