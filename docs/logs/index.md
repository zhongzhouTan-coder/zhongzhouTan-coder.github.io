...
- [Recurrent Neural Networks: From RNN to LSTM](../algorithms/recurrent-neural-networks/index.md) — Sequence processing through shared recurrent weights and hidden state, the long-term dependency problem, LSTM gating, and the conceptual bridge to linear attention's RNN mode.
...
- [Megatron-LM: GPU-Cluster Training Parallelism](../training/megatron-lm/index.md) — Covers both Megatron-LM papers: intra-layer tensor parallelism with f/g conjugate operators and column-parallel GEMM splitting (2019), and PTD-P recipe for trillion-parameter GPT models (2021).
