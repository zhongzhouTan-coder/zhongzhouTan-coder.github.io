...
- [Perplexity (PPL): From Next-Token Loss to Reliable Evaluation](perplexity.md) — Explains exponentiated average next-token negative log likelihood, a worked example, fixed-length sliding-window evaluation, and tokenizer/protocol comparison caveats.
- [Gray Code: One-Bit Transitions for Safe State Changes](gray-code/index.md) — Binary-reflected Gray code, one-bit adjacency, reflect-and-prefix construction, shift/XOR conversion, and transition-safe hardware and communication uses.
