...
- [FlashAttention-3: Hopper Asynchrony and FP8 Attention](flashattention/flashattention-3.md) — Hopper attention kernel design: warp specialization, TMA/WGMMA asynchrony, GEMM-softmax overlap, FP8 block quantization, incoherent processing, Big Picture pipeline diagram, landscape of Hopper hardware exploitation, and speed/accuracy results.
...
- [DeepSeek-V2 Multi-Head Latent Attention](attention-variants/deepseek-v2-mla.md) — DeepSeek-V2's MLA design: low-rank joint K/V latent cache, decoupled RoPE side channel, query compression, MoE serving context, and reported 93.3% KV-cache reduction with 5.76× maximum generation throughput versus DeepSeek 67B.
